...
const ProductCard = ({ product }) => {
  ...
  return (
    <motion.div 
      whileHover={{ y: -4 }}
      className="bg-white border border-fresh-green/10 rounded-2xl p-4 shadow-sm hover:shadow-lg transition-all relative"
    >
      {/* Discount Badge */}
      {discount > 0 && (
      ...
      >
        <Heart
          size={16}
          className={isWishlisted ? 'text-red-500 fill-red-500' : 'text-fresh-green/60'}
          strokeWidth={2.5}
        />
      </motion.button>
      ...
      <div className="mt-3">
        {quantity === 0 ? (
          <motion.button
            ...
            className="w-full bg-white border-2 border-leaf text-leaf font-bold text-sm py-2 rounded-lg hover:bg-leaf hover:text-white transition-all"
          >
            ADD
          </motion.button>
        ) : (
          <div className="flex items-center justify-between bg-leaf rounded-lg p-1">
            <motion.button
              whileTap={{ scale: 0.9 }}
              onClick={() => removeFromCart(product.id)}
              className="bg-white text-leaf rounded-md p-1.5 hover:bg-fresh-green/10 transition-colors"
            >
